...
run( 'node', [ '-e', `require('fs').copyFileSync(${ JSON.stringify( sql ) }, '.wp-env/uploads/shop.sql')` ] );
run( 'pnpm', [ 'wp-env', 'run', 'cli', 'wp', 'db', 'reset', '--yes' ] );
run( 'pnpm', [ 'wp-env', 'run', 'cli', 'wp', 'db', 'import', '/var/www/html/wp-content/uploads/shop.sql' ] );

if ( existsSync( media ) ) {
	run( 'node', [ '-e', `require('fs').copyFileSync(${ JSON.stringify( media ) }, '.wp-env/uploads/uploads.tar.gz')` ] );
	run( 'pnpm', [ 'wp-env', 'run', 'cli', 'sh', '-c', '"cd /var/www/html/wp-content/uploads && tar xzf uploads.tar.gz"' ] );
}

run( 'pnpm', [ 'wp-env', 'run', 'cli', 'wp', 'rewrite', 'flush' ] );
run( 'pnpm', [ 'wp-env', 'run', 'cli', 'wp', 'wc', 'tool', 'run', 'regenerate_thumbnails', '--user=admin' ] );
